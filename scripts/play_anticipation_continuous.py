...
def make_buffer_name(
    *,
    ...
    return f"anti_wrist_v4_{digest}"
...
def build_continuous_trajectory(
    ...
    second_left_no_wrist = [
        ...
        for center_value, left_value
        in zip(center, left)
    ]

    # 最后一次左看时，wrist3保持正中角度
    second_left_no_wrist[
        wrist3_index
    ] = center_wrist3
    ...
    # 第二次左看的时间计算也使用半程点。
    # 距离和时间同时减半，因此转动速度保持不变。
    second_left_timing = list(
        second_left_no_wrist
    )
    ...
    # SECOND_LEFT_WRIST_LOCK_TANGENTS
    # 关键点编号：
    # 5 = 右看结束后的正中
    # 6 = 第二次左看
    # 7 = 第二次左看结束后的正中
...
def main() -> int:
    ...
    try:
        with AuboSdkClient(config) as client:
            ...
            print()
            print("等待机械臂真正到达最终低头点……")

            final_error = wait_until_pose_reached(
                client,
                look_down,
                timeout=max(
                    30.0,
