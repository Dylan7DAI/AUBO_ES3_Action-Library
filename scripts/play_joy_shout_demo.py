...
def main() -> int:
    ...
    if not 0.20 <= args.approach_velocity <= 0.60:
        print(
            "approach-velocity必须在0.20到0.60之间。",
            file=sys.stderr,
        )
        return 2

    if not 0.35 <= args.shout_velocity <= 0.90:
        print(
            "shout-velocity必须在0.35到0.90之间。",
            file=sys.stderr,
        )
        return 2

    if not 0.30 <= args.return_velocity <= 0.80:
        print(
            "return-velocity必须在0.30到0.80之间。",
            file=sys.stderr,
        )
        return 2
