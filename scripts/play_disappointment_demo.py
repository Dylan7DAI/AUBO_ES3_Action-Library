...
def main() -> int:
    ...
    try:
        ...
        one_cycle = breathing.build_breathing_trajectory(
            retreat,
        ...
        for cycle_index in range(args.breath_cycles):
            if cycle_index < args.breath_cycles - 1:
                trajectory.extend(
                    list(point)
        ...
        # 两次呼吸结束后，在受挫中心停稳0.60秒。
        post_hold_seconds = 0.60
        ...
        # 在同一条路径缓存中，
        # 从retreat平滑衔接到anticipation_look_down。
        #
        # 五次平滑曲线保证起点与终点速度、加速度接近0。
        maximum_delta = max_difference(
            retreat,
            lookdown,
        )

        transition_velocity = 0.22
        ...
        signature = {
            "cycles": args.breath_cycles,
            "amplitude": args.breath_amplitude_deg,
            "cycle_seconds": args.breath_cycle_seconds,
            "post_hold": post_hold_seconds,
