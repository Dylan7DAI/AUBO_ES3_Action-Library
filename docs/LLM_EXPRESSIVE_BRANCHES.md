...
| 方案 | 它比现有方案多做了什么 | 动作是怎么生成的 | 主要好处 | 主要劣势 |
|---|---|---|---|---|
| **方案一：固定轨迹连续参数化** | 现有方案只能从 0–3 四档强度中选一档；方案一让 LLM 分别调节动作有多大、多快、多有力、停多久和重复几次。 | 动作姿势和先后顺序不变，程序只按照 LLM 给出的连续参数，对已经示教好的动作进行放大、缩小、加速或减速。 | 五个方案中改动最小、风险最低，也最容易解释和上真机验证；动作变化比固定四档更细。 | 动作骨架始终相同，最后可能只是“同一个动作更大或更快”，不同情感的动作形状仍然比较相似；部分参数还没有真正影响执行。 |
| **方案二：统一 Laban 动作参数** | LLM 不再只说强度，而是描述动作应该上扬还是下沉、舒展还是收缩、突然还是平缓、直接还是弯曲。 | 程序把这些 Laban 风格参数（Effort，Shape, Space），当成几个“旋钮”，用它们混合少量已经示教好的高兴、失望和侧向姿势。 | 同一套风格参数可以描述多种情感；用较少的示教姿势就能组合出更多变化；每个参数也比较容易用动作理论解释。 | “风格词”怎样变成机械臂运动仍由设计者手工规定；`freedom`、`lightness` 等参数目前没有真正影响动作；参数不同，人眼也不一定能明显看出差别。 |
| **方案三：参考动作加受限笛卡尔残差** | LLM 先使用一个正向或负向参考动作，再提出小幅修改，例如再抬高一点、侧移一点、回撤一点或转动手腕一点。 | 程序把归一化修改限制在几厘米和几度以内，叠加到参考动作的末端位置上，再生成 TCP 路径，并通过 IK 换算成关节动作。 | 保留人工示教动作的基本风格，同时能产生比单纯调速度、幅度更明显的空间差异；比完全从零生成动作更受约束。 | 会生成新的空间路径，因此增加了 IK 失败、关节跳变、奇异姿态和碰撞风险；依赖参考动作与坐标方向是否合理；当前默认不能上真机，只能做 FK/IK 验证。 |
| **方案四：归一化末端关键帧与连续 IK** | LLM 不再修改现成动作，而是提出“准备点”和“情绪最高点”应该大致在哪里、朝向哪里，以及两点之间走什么路径。 | 程序把 LLM 的抽象方向和比例换算成真实 TCP 关键帧，补出中间路径点，再逐点通过 IK 计算关节角。 | 五个方案中动作创作空间最大，有机会产生示教库里没有的新动作，也不需要为每种情感提前录制完整动作。 | 风险和实现难度最高，最容易出现不自然姿势、IK 失败或碰撞；目前逐点 `moveJoint` 也可能不够流畅；补齐碰撞模型和更合适的连续执行方式前不能上真机。 |
| **方案五：多候选与本地风险感知选择** | 现有方案每轮只生成一个结果；方案五让 LLM 同时提出三个侧重点不同的候选动作，再由本地程序决定用哪一个。 | 三个候选分别强调动作形状、节奏和路径；程序逐个编译和做安全检查，再根据情感是否合适、是否与上一轮不同、是否平滑以及安全余量进行评分，选择得分最高的动作。 | 更容易兼顾多样性与安全性；一个候选失败时还可以尝试其他候选；最终选择权在本地程序，LLM 不能指定或绕过安全规则。 | 每轮生成和检查三个动作，耗时与计算量更高；评分规则和权重仍是人工设计；三个候选可能很相似，也可能全部失败，而且程序认为“新颖”不代表用户一定能感受到差异。 |
